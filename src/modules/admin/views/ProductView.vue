  ...
  <form @submit="onSubmit" class="grid grid-cols-1 sm:grid-cols-2 bg-white px-5 gap-5">
    <div class="first-col">
      ...
      <div class="mb-4">
        ...
        <!-- <input v-model="title" v-bind="titleAttrs" type="text" id="title" :class="[
          'form-control',
          {
            'border-red-500': errors.title,
          },
        ]" />
        <span class="capitalize text-red-400" v-if="errors.title">{{ errors.title }}</span> -->
      ...
      <div class="mb-4">
        ...
        <div class="flex ">
          <button v-for="size of allSizes" :key="size" @click="toggleSize(size)" type="button" :class="[' p-2 rounded w-14 mr-2 flex-1', {
    ...
    <div class="first-col">
      ...
      <div class="my-4 text-right">
        <button :disabled="isPending" type="submit"
          class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded disabled:bg-gray-300">
          Guardar
        </button>
      </div>
    </div>
