...
const onLogin = async () => {
  ...
  if (myForm.rememberMe) { //si esta en true
    localStorage.setItem('email', myForm.email) //quieroi grabar el email en el localstorage
  } else {
    localStorage.removeItem('email') //sino, lo remuevo
  }

  const ok = await authStore.login(myForm.email, myForm.password) //le mando email y password
